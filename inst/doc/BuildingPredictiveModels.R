...
| Multilayer Perceptron | Neural networks contain multiple layers that weight their inputs using a non-linear function.  The first layer is the input layer, the last layer is the output layer, and the layers between are the hidden layers.  Neural networks are generally trained using feed forward back-propagation.  This is when you go through the network with a data-point and calculate the error between the true label and predicted label, then go backwards through the network and update the linear function weights based on the error.  | size (the number of hidden nodes), alpha (the l2 regularisation), seed |
| Support vector machine | Support vector machines learn a separating hyperplane between outcome classes and can use kernels to capture non-linear relationships. The implementation uses scikit-learn's SVC in Python. | C, gamma, kernel, classWeight, seed |
| LightGBM | LightGBM is a gradient boosting framework based on decision trees and can be efficient for large sparse data sets. | numLeaves, learningRate, minDataInLeaf, nrounds, seed |
